...
cluster <- parallel::makeCluster(threads_parallel, type = "FORK")
#Export cluster
parallel::clusterExport(cl = cluster,
                        c("allele_diff", "DATA", "VGERM"),
                        envir =
                          environment())
...
DATA$mut_v <-
  parSapply(cluster, 1:nrow(DATA), function(i) {
    v_seq <-
      substr(DATA$sequence_alignment[i], 1, DATA$v_germline_end[i])
    v_call <- strsplit(DATA$v_call[i], ",")[[1]][1]
    germ_seq <- VGERM[[v_call]]
    length(allele_diff(c(v_seq, germ_seq)))
...
dbs_filter <-
  dbs %>% filter(!grepl("^[.]", sequence_alignment)) %>% rowwise() %>% mutate(
  ...
  ) %>% filter(!grepl("N", v_seq),!grepl("-", v_seq), nchar(v_seq) > 312)
...
dbs_filter <- data.table::setDT(DATA)
dbs_filtered_mut3 <-
  dbs_filter[mut_v <= 3 &
               !grepl("^[.]", sequence_alignment) &
               v_germline_end > 312]

dbs_filter <- setDT(dbs_filter)
dbs_filter <-
  dbs_filter[, mut := allele_diff(v_seq, v_seq_germ), by = seq_len(nrow(dbs_filter))]
...
data_[, v_call_new := stringi::stri_replace_all_regex(
  str = v_call,
  pattern = paste0(gsub("[*]", "[*]", alleles_db$or_allele), "\\b"),
  replacement = alleles_db$new_allele,
  vectorize_all = F
)]
data_[, v_gene := alakazam::getGene(v_call_new,
                                    first = F,
                                    collapse = T,
                                    strip_d = F)]
data_[, .(count = .N, multi = sum(grepl(",", v_gene))), by = v_gene]
data_ <- data_[!grepl(",", v_gene)]
data_[, v_allele := stringr::str_replace_all(
  gsub(
...
dbs_filter$mut <-
  parSapply(cluster, 1:nrow(dbs_filter), function(i) {
    length(allele_diff(c(
      dbs_filter$v_seq[i], dbs_filter$v_seq_germ[i]
    )))
  })
...
data_multi <-
  data_multi[, .(v_gene2 = unlist(tstrsplit(v_gene2, ",", type.convert = FALSE))), by = setdiff(names(data_multi), "v_gene2")]
data_multi_sum <-
  data_multi[, .(count = .N, multi = sum(grepl(",", v_gene))), by = list(v_gene2, subject)]

data_filter <- data_[!grepl(",", v_gene)]
data_filter[, v_allele := stringr::str_replace_all(
  gsub(
    "(IG[HKL][VDJADEGMC]|TR[ABDG])[A-Z0-9\\(\\)]+[-/\\w]*[*]",
    "",
    v_call_new,
...
data_multi_a <-
  data_multi_a[, .(v_allele2 = unlist(tstrsplit(v_allele2, ",", type.convert = FALSE))), by = setdiff(names(data_multi_a), "v_allele2")]
data_multi_a_sum <-
  data_multi_a[, .(count = .N, multi = sum(grepl(",", v_allele))), by = v_allele2]
...
allele_groups <- function(tmp, genes, gr) {
  ...
  diag(tmp) = NA
  val <-
    tmp[rowSums(tmp == 0, na.rm = T) != 0, colSums(tmp == 0, na.rm = T) != 0]
  ...
  n <- length(grep("_",f_alleles,invert = T))
  for (x in 1:n) {
    new <- paste0(gene, "G", gr, "*", ifelse(x < 10, "0", ""), x)
    new_alleles <-
      dplyr::bind_rows(
        data.frame(
          or_allele = grep("_",f_alleles,invert = T,value = T)[x],
          new_allele = new,
          remove = F,
          stringsAsFactors = F
        ),
  ...
  if (length(rm_a) != 0)
    ...
    dplyr::bind_rows(
      data.frame(
        or_allele = rm_a,
        new_allele = new_alleles$new_allele[new_alleles$or_allele %in% unique(rm_s)],
        remove = T,
        stringsAsFactors = F
      ),
      new_alleles
  ...
  n <- length(grep("_",f_alleles))
  if(n!=0){
    for(x in 1:n){
      
      base <- strsplit(grep("_",f_alleles,value = T)[x],"_")[[1]][1]
      
      new_allele <- new_alleles$new_allele[new_alleles$or_allele == base]
      
      new <- gsub(base,new_alleles,grep("_",f_alleles,value = T)[x])
      
      new_alleles <-
        dplyr::bind_rows(
          data.frame(
            or_allele = new_alleles,grep("_",f_alleles,value = T)[x],
            new_allele = new,
            remove = F,
            stringsAsFactors = F
          ),
...
cut <-
  data.frame(cluster = dendextend::cutree(as.dendrogram(hc_sub, hang = -1), h = range_seq))
hc_sub <- cut
...
hc_sub$IMGT <-
  imgt[getGene(rownames(hc_sub), strip_d = F, omit_nl = F)]

hc_sub$labels <- factor(rownames(hc_sub), levels = row_order)
hc_sub$labels_ind <-
  setNames(1:length(levels(hc_sub$labels)), levels(hc_sub$labels))[hc_sub$labels]
hc_plot <-
  reshape2::melt(hc_sub, id.vars = c("labels", "labels_ind"))

hc_plot$freq <- 1

hc_plot$variable <-
  factor(hc_plot$variable, levels = c("IMGT", paste0((1 - range_seq) * 100, "%")))
...
for (gr in unique(groups)) {
  ...
  if (length(unique(getGene(
    genes, strip_d = F, omit_nl = F
  ))) == 1) {
    new <-
      sapply(genes, function(x)
        paste0(strsplit(x, "[*]")[[1]][1], "G", gr, "*", strsplit(x, "[*]")[[1]][2]))
    alleles_db <-
      bind_rows(alleles_db,
                data.frame(
                  or_allele = genes,
                  new_allele = new,
                  remove = F
...
tmp <-
  tmp %>% group_by(cluster) %>% dplyr::summarise(
    gene = paste0(unique(alakazam::getGene(alleles, strip_d = F)), collapse = "/"),
...
ids <-
  which(!names(vgerms$IGH) %in% alakazam::getAllele(
    names(vgerms_full$IGH),
    strip_d = F,
    omit_nl = F
  ))
for (i in ids) {
  tmp <- vgerms_full$IGH[1]
...
for (f in files) {
  ...
  g_names <-
    sapply(g_names, function(x)
      paste0(sort(strsplit(x, "/")[[1]]), collapse = "/"))
  if (grepl("/", g_gene))
    g_gene <- paste0(sort(strsplit(g_gene, "/")[[1]]), collapse = "/")
  ...
  if (length(g_id_new) > 1) {
    ...
    if (paste0(g_gene, g_id) == g_id_new) {
      file.copy(f,
                "~/Dropbox (BIU)/genotype_sub_sampling/sister_genes/reference_book2/")
    } else{
      # change first line
      lines[1] <-
        paste0(strsplit(id, " - ")[[1]][1],
               " - ",
               "G",
               strsplit(g_id_new, "[0-9]G")[[1]][2])
      # gerp line of g_group
      lines[grep("g_group =", lines)] <-
        gsub(paste0(g_gene, g_id), g_id_new, lines[grep("g_group =", lines)])
      # write file
      writeLines(
        lines,
        paste0(
          "~/Dropbox (BIU)/genotype_sub_sampling/sister_genes/reference_book2/",
          ifelse(
            as.numeric(strsplit(g_id_new, "[0-9]G")[[1]][2]) < 10,
            paste0('0', as.numeric(strsplit(
              g_id_new, "[0-9]G"
            )[[1]][2])),
            as.numeric(strsplit(g_id_new, "[0-9]G")[[1]][2])
          ),
          "-G",
          as.numeric(strsplit(g_id_new, "[0-9]G")[[1]][2]),
          ".rmd"
...
for (i in 1:51) {
  if (any(grepl(paste0("G", i, "[.]"), files2))) {
    ...
    # gerp line of g_group
    lines[grep("g_group =", lines)] <-
      gsub("IGHV4-4G40", g_group, lines[grep("g_group =", lines)])
    # write file
    writeLines(
      lines,
      paste0(
...
### sort absolute dict
alleles_db$gene <-
  getGene(alleles_db$or_allele, strip_d = F, omit_nl = F)
alleles_db$gene_group <-
  getGene(alleles_db$new_allele,
          strip_d = F,
          omit_nl = F)
...
for (i in 1:length(absolute_thresholds_dict)) {
  ...
  g_new <-
    unique(alleles_db$gene_group[alleles_db$or_allele %in% alleles])
  print(g_new)
  print("")
  print("###############")
  if (length(g_new) > 1) {
...
dfs <-
  data.table::rbindlist(
    lapply(absolute_thresholds_dict, function(l)
      data.frame(
        or_allele = paste0("IGH", names(l)),
        thresh = unlist(l),
        stringsAsFactors = F,
        row.names = NULL
      )),
...
dfs_f <-
  dfs_f %>% dplyr::group_by(new_allele) %>% dplyr::summarise(
    func_group = func_group,
    or_allele = paste0(or_allele, collapse = "/"),
